...
exports.createPaymentIntent = functions.https.onCall(async (data, context) => {
    ...
    const doc = await db.collection(`stripe_paymentIntents`)
        .where("customer", "==", customerID)
        .where("status", "!=", "succeeded").get()
    if (!doc.exists) {
        const intentInfo = await stripe.paymentIntents.create(
            {
                amount: 0,
                currency: 'usd',
        ...
        await db.doc(`stripe_paymentIntents/${intentInfo.id}`).set({ ...intentInfo })
    }
