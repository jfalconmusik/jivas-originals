...
exports.createStripeCustomer = functions.auth.user().onCreate(async (user) => {
    ...
    return admin.firestore().collection('stripe_customers').doc(user.uid).set({ customer_id: customer.id, customer_email: user.email });
});


exports.createPaymentIntent = functions.firestore.document('/stripe_paymentIntents/{intentId}').onCreate(async (snap) => {
    ...
    return admin.firestore().collection('stripe_paymentIntents')
        .doc(snap.ref).set(
            {
                ...paymentIntent
            })
